...
tokentypes token_type(char *t, int start){
	if (isalpha(*t)){return VAR;}
	else if (*t == '(' || *t == ')' || *t == '*'|| *t == '/' || *t == '^' || *t == '+' ){
	         	return CTL;}
	else if (*t == '-'){
		if (start) {return CONST;}
		t--;
		if (isdigit(*t++)){
			return CTL;
		} else {
...
node * lex (char *input) {
	remove_spaces(input);
	unsigned short i = 0;
	node *cur = makenode(); node *head = makenode();
	head = cur; cur->prev = NULL;
	int start = 1;
	
	while(*input != '\0'){
		node *new = makenode();
		tokentypes type = token_type(input, start);
